            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      !data.time.startDate && "text-muted-foreground"
                    )}
                  >
                    {data.time.startDate
                      ? format(new Date(data.time.startDate), "PPP p")
                      : "Pick a start date"}
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={data.time.startDate ? new Date(data.time.startDate) : undefined}
                    onSelect={(date) => {
                      setData("time", {
                        ...data.time,
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "w-full justify-start text-left font-normal",
                      !data.time.endDate && "text-muted-foreground"
                    )}
                  >
                    {data.time.endDate
                      ? format(new Date(data.time.endDate), "PPP p")
                      : "Pick an end date"}
                    <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={data.time.endDate ? new Date(data.time.endDate) : undefined}
                    onSelect={(date) => {
                      setData("time", {
                        ...data.time,
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "w-full justify-start text-left font-normal",
                      !data.time[0] && "text-muted-foreground"
                    )}
                  >
                    {data.time[0]
                      ? format(new Date(data.time[0]), "PPP p")
                      : "Pick a start date"}
                    <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={data.time[0] ? new Date(data.time[0]) : undefined}
                    onSelect={(date) => {
                      setData("time", {
                        ...data.time,
                        0: date?.toISOString() || "",
