...
const EditAreaSheet = ({ onOpenChange, selected, ...props }: Props) => {
    const { data, setData, put, errors, reset, processing } = useForm<FormType>({
        name: selected.name,
        districts: selected.districts.split(',').map((district: any) => ({ value: district.trim() })),
    });
    ...
    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        put(route('areas.update', selected.id), {
            onSuccess: () => {
